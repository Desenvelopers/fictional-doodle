...
class viagemRepository {
    static async getTravellerIdByDateAndCity( dataInicial: Date, cidadeDestinatario, cidadeRemetente ) {
        ...
        return await viagemModel.findOne( { diaInicio: { $gt : day}, mes: month, cidadePartida: cidadeRemetente, cidadeChegada: cidadeDestinatario }, 'idTraveller' ).sort( '-diaInicio' ).exec()
    }
}
